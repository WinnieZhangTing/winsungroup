...
export async function generateStaticParams() {
  const slugs = await client.fetch<string[]>(
    groq`*[_type == 'case-study.post' && defined(metadata.slug.current)].metadata.slug.current`,
  )
...
async function getPost(params: { slug?: string }) {
  return await fetchSanity<Sanity.CaseStudyPost>(
    groq`*[_type == 'case-study.post' && metadata.slug.current == $slug][0]{
      ...,
...
async function getPageTemplate() {
  return await fetchSanity<Sanity.Page>(
    groq`*[_type == 'page' && metadata.slug.current == 'case-study/*'][0]{
      ...,
