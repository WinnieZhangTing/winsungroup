...
export async function generateStaticParams() {
  const slugs = await client.fetch<string[]>(
    groq`*[_type == 'news.post' && defined(metadata.slug.current)].metadata.slug.current`,
  )
...
async function getPost(params: { slug?: string }) {
  return await fetchSanity<Sanity.NewsPost>(
    groq`*[_type == 'news.post' && metadata.slug.current == $slug][0]{
      ...,
...
async function getPageTemplate() {
  return await fetchSanity<Sanity.Page>(
    groq`*[_type == 'page' && metadata.slug.current == 'news/*'][0]{
      ...,
